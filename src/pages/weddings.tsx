...
const Weddings = () => {
  const midpoint = Math.floor(weddingGalleryImages.length / 2);
...
<section id="gallery" className="py-20 px-6 md:px-12 lg:px-16">
  ...
  <div key={`bottom-${index}`} className="w-72 aspect-[2/3] flex-shrink-0 overflow-hidden rounded-md relative">
    <img
      src={image.src}
      alt={image.alt}
      className="absolute inset-0 w-full h-full object-cover"
