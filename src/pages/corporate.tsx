    ...
    <>
      <div className="min-h-screen bg-transparent flex flex-col">
        ...
        <section
          ...
          style={{ backgroundImage: "url('/lovable-uploads/corp3_cropped.png')" }}
        >
        ...
        <section id="what-we-do" className="py-20 px-6 md:px-12 lg:px-16 bg-[#2a2a2a] text-white">
          <div className="text-center mb-16">
            ...
            <p className="text-lg max-w-3xl mx-auto">
              Planning an Atlanta-based, high-stakes corporate event means juggling timelines...
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {[{ title: "Tight timelines?", short: "Delivered.", icon: Check }, { title: "Global audiences?", short: "Understood.", icon: Globe }, { title: "Deadline meltdowns?", short: "Handled.", icon: Users }].map(({ title, short, icon: Icon }, idx) => (
              <div key={idx} className="flex flex-col items-center text-center p-6 bg-white shadow-sm rounded-lg">
                ...
                <p className="text-lg text-[#7b6535]">{short}</p>
              </div>
            ))}
          </div>
        </section>
              ...
              {["Define the Mission", "Build the Blueprint", "Execute with Excellence"].map((title, index) => (
                <div key={title} className="flex flex-col md:flex-row items-start gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-lg">
                      {index === 0 && "We begin with your purpose and goals — then map backward..."}
                      {index === 1 && "We manage vendors, tech, and timelines..."}
                      {index === 2 && "We run the event so you can focus on your audience..."}
                    </p>
                  </div>
                </div>
              ))}
